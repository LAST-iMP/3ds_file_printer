...
class Chunk {
public:
    Chunk(unsigned short i, fstream& fp);
    virtual ~Chunk();

    const unsigned short id;
    const unsigned int length;

    unordered_map<unsigned short, vector<Chunk*>*>& getSubChunks() {return subChunks;}
    ...
    Chunk* getFirstSubChunk(unsigned short key) {
        return (*subChunks[key])[0];
    }

protected:
    unordered_map<unsigned short, vector<Chunk*>*> subChunks;

    void addSubChunk(int id, Chunk* chunk);
    unsigned int getSubChunkLength();

    template<typename T>
    T* generate(unsigned short i, fstream &fp) {
        return new T(i, fp);
    }


protected:
    static unsigned short readShort(fstream & fp);
    static unsigned int readInt(fstream & fp);
    static float readFloat(fstream & fp);
};
